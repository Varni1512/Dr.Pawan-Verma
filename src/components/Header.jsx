        ...
        <nav className="hidden md:flex items-center space-x-2 lg:space-x-4 flex-grow justify-end">
          {mainNavItems.map((link) => {
            ...
            return (
              <a
                key={link.href}
                href={link.href}
                className={`relative px-2 lg:px-3 py-2 transition duration-300 text-[28px] ${isActive ? "text-yellow-500" : "hover:text-yellow-500"}`}
              >
                {link.label}
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className={`flex items-center px-2 lg:px-3 py-2 transition duration-300 text-[28px] cursor-pointer ${isMoreItemActive ? "text-yellow-500" : "hover:text-yellow-500"}`}
            >
                ...
                {moreNavItems.map((link) => {
                  ...
                  return (
                    <a
                      key={link.href}
                      href={link.href}
                      className={`block px-4 py-2 transition duration-300 text-[28px] ${isActive ? "text-yellow-500" : "hover:text-yellow-500 hover:bg-gray-900"}`}
                    >
                      {link.label}
                    </a>
                  );
          ...
          {allNavItems.map((link) => {
            ...
            return (
              <a
                key={link.href}
                href={link.href}
                className={`relative block px-4 py-2 transition duration-300 text-[28px] ${isActive ? "text-yellow-500" : "hover:text-yellow-500"}`}
              >
                {link.label}
                {isActive && (
